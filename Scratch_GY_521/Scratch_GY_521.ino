...
//modify the following lines according to which Arduino pins are used
//Analogue inputs
const int sliderPin = A0;//NB doubling up with resA
const int lightPin = A4;
const int soundPin = A5;
const int resAPin = A0;
const int resBPin = A1;
const int resCPin = A2;
const int resDPin = A3;
//digital input
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  float angle_x_positiv = (angle_x+100)/2;
  float angle_y_positiv = (angle_y+100)/2;
  
  unsigned int angle_x_scratch = (angle_x_positiv);
  unsigned int angle_y_scratch = (angle_y_positiv);

  angle_x_scratch = angle_x_scratch*10;
  angle_y_scratch = angle_y_scratch*10;
...
//  delay(5);
    //Serial.println(angle_x_positiv);
    //Serial.println(angle_y_positiv);
    //Serial.print("x");
    //Serial.print(angle_x_scratch);
    //Serial.print("y");
    //Serial.println(angle_y_scratch);
    //delay(200);
  ...
  if(Serial.available()>0){
    if (Serial.read() == 1){ 
      //scratchResponse();
      //according to the tech spec we need to send a firmware vsn first. A bogus firmware = 128+1
      transmitValue(15, 129);
      //now sensor values
      //transmitValue(0, analogRead(resAPin));//resistance-A
      //transmitValue(1, analogRead(resBPin));//resistance-B    
      //transmitValue(0, 1023);//resistance-B    
      //transmitValue(1, 1023);//resistance-B    
      transmitValue(0, (angle_x_scratch));//resistance-A
      transmitValue(1, (angle_y_scratch));//resistance-A
      transmitValue(2, analogRead(resCPin));//resistance-C
      transmitValue(3, analogRead(resDPin));//resistance-D
      transmitValue(4, analogRead(sliderPin));//slider
      transmitValue(5, analogRead(lightPin));//light
      //transmitValue(6, updateSoundWindow(readScaledSound()));//sound
      transmitValue(6, analogRead(resBPin));//resistance-C
      transmitValue(7, digitalRead(buttonPin)?1023:0);//button
    }
  }
}
...
/*
Respond to a Scratch polling event with all sensor data
 */
//void scratchResponse(){
//  //according to the tech spec we need to send a firmware vsn first. A bogus firmware = 128+1
//  transmitValue(15, 129);
//  //now sensor values
//  transmitValue(0, analogRead(resAPin));//resistance-A
//  transmitValue(1, analogRead(resBPin));//resistance-B    
//  //transmitValue(0, angle_x);//resistance-A
//  //transmitValue(1, angle_y);//resistance-A
//  transmitValue(2, analogRead(resCPin));//resistance-C
//  transmitValue(3, analogRead(resDPin));//resistance-D
//  transmitValue(4, analogRead(sliderPin));//slider
//  transmitValue(5, analogRead(lightPin));//light
...
/*
Transmit the 2-bytes of data that identify the sensor and its value
 */
void transmitValue(int sensor, int value)
{
  //delays as per the scratchboard tech spec R2
